...
componentsDirs?.forEach(directory => {
    ...
    const uiPathInDirectory = path.resolve(__dirname, "..", "..", "src", "shared", "ui", directory.getBaseName(), "ui")
    // const isUiInDirectory = directory.getDirectory("ui")
    const isUiInDirectory = directory.getDirectory(uiPathInDirectory)
    ...
    if (!indexFile) {
        ...
        const sourceCode = isUiInDirectory ? exportFromUi : exportFromFile
...
function isAbsolute(value: string) {
    const folderNamesArr = ["app", "shared", "entities", "pages", "features", "widgets"]
